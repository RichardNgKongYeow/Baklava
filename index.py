...
def main():
    ...
    try:
        loop.run_until_complete(
            asyncio.gather(
                baklava_client.log_event_listener_loop(
                    baklava_client.create_mst_event_filter(), 2, myQueue),
                baklava_client.log_event_listener_loop(
                    baklava_client.create_bst_event_filter(), 2, myQueue),
                Clients.marginx_log_event_executer_loop(
                    client_list, 2, myQueue),
            ))
